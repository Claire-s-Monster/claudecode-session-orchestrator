...
class ComplianceVerifier:
    ...
    async def verify_full_compliance(self) -> ComplianceResult:
        ...
        for check_result in await asyncio.gather(*checks, return_exceptions=True):
            ...
            # Fix: Check if check_result is dict before accessing attributes
        ...
        try:
            ...
            if env_file.exists():
                ...
                if (
                    ...
                    or ".env" not in gitignore_file.read_text()
                ):
                    violations.append(
                        ComplianceViolation(
    ...
    async def _check_framework_adherence(self) -> dict:
        ...
        try:
            ...
            existing_dirs = [
                d for d in expected_dirs if (self.project_root / d).exists()
            ]
    ...
    async def _run_quality_command(self, command: str) -> dict | None:
        ...
        return {
            "compliance_report": {
                ...
                "framework_compliance": {
                    "mcp_first_strategy": result.metrics.get("mcp_percentage", 0)
                    >= self.mcp_usage_target,
                    "zero_tolerance_quality": len(
                        [
                            v
                            for v in result.violations
