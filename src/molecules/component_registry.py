...
Atomic design: molecule-level. Python 3.12+, async/await patterns.
"""
...
class ComponentRegistry:
    ...
    async def register(
        ...
        *,
        ...
        Args:     component: The component instance to register.     name: Optional name
        for the component (default: component.__class__.__name__).     type_hint:
        Optional explicit type for registration (default: type(component)).
        overwrite: If True, overwrite existing registration with same type/name.

        Raises:     ComponentRegistrationError: If already registered and overwrite is
        False.
        """
    ...
    async def deregister(
        ...
        *,
        ...
        Args:     name: Name of the component.     type_hint: Type of the component.

        Raises:     ComponentNotFoundError: If component not found.
        """
        async with self._lock:
    ...
    async def get(
        ...
        Args:     type_hint: The type of the component.     name: Optional name of the
        component.

        Returns:     The component instance.

        Raises:     ComponentNotFoundError: If not found.
        """
        async with self._lock:
            if type_hint not in self._components:
                raise ComponentNotFoundError(
    ...
    async def find_all(
        ...
        Args:     type_hint: Optional type to filter by.
    ...
    async def inject(
        ...
        Usage:     @registry.inject(MyType)     async def my_func(dep):         ...

        The decorated function will receive the discovered component as its first
        argument.

        Args:     type_hint: The type of the component to inject.     name: Optional
